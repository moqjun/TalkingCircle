...
@interface EaseMessageViewController ()<EaseMessageCellDelegate,TCRedpacketSendDelgate>
{
...
#pragma mark - TCRedpacketSendDelgate

-(void)personalRedpacketSendWithRedpacketInfo:(NSMutableDictionary *)redpacketInfoDic
{
    [redpacketInfoDic safeSetObject:@"redPacket" forKey:@"type"];
    [self sendTextMessage:@"redPacket[点我领红包]" withExt:redpacketInfoDic];
}
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        ...
        {
            _scrollToBottomWhenAppear = NO;
            if ([[model.message.ext objectForKey:kTCRedpacketTypeKey] isEqualToString:kTCRedpacketIdentificationString]) {
                TCGetRedPacketView *redpacketView = [[TCGetRedPacketView alloc] initWithFrame:self.view.bounds];
                [redpacketView setRedpacketInfo:model.nickname userIconUrl:model.avatarURLPath userIconImage:model.avatarImage message:nil orderId:nil];
                [redpacketView show];
            }
        }
            break;
...
- (void)moreViewReadPacketAction:(EaseChatBarMoreView *)moreView
{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
    
    TCRedPacketSendController *controller = [[TCRedPacketSendController alloc] init];
    controller.delegate = self;
    controller.userId = self.userModel.buddy;
    
    [self presentViewController:controller animated:YES completion:^{
        
    }];
//    UINavigationController *redPacketNavigationController = [[UINavigationController alloc] initWithRootViewController:controller];
//    [self.navigationController presentViewController:redPacketNavigationController animated:YES completion:^{
//        
//    }];
//    NSDictionary *ext = [NSDictionary dictionaryWithObjectsAndKeys:@"readPacket",@"isRead", nil];
//    [self sendTextMessage:@"redPacket[点我领红包]" withExt:ext];
}
...
- (void)moreViewCardAction:(EaseChatBarMoreView *)moreView
{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
    NSDictionary *ext = [NSDictionary dictionaryWithObjectsAndKeys:@"card",@"isRead", nil];
    [self sendTextMessage:@"card[名片]" withExt:ext];
}
