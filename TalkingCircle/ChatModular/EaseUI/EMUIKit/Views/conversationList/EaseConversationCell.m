...
+ (void)initialize
{
    ...
    cell.titleLabelFont = [UIFont systemFontOfSize:19];
    cell.detailLabelColor = [UIColor colorWithHexString:@"555"];
    cell.detailLabelFont = [UIFont systemFontOfSize:15];
    cell.timeLabelColor = [UIColor colorWithHexString:@"888"];
    cell.timeLabelFont = [UIFont systemFontOfSize:15];
}
...
- (void)_setupSubview
{
    ...
    _avatarView.imageCornerRadius = 25.0;
...
//    添加消息免打扰图标
    _msgFreeImageView = [[UIImageView alloc] init];
    _msgFreeImageView.translatesAutoresizingMaskIntoConstraints = NO;
    _msgFreeImageView.image = [UIImage imageNamed:@"home-clock"];
    _msgFreeImageView.hidden = YES;
    [self.contentView addSubview:_msgFreeImageView];
    ...
    [self _setupMsgFreeImageViewConstraints];
}
...
{
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.timeLabel attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:EaseConversationCellPadding/2]];
...
-(void)_setupMsgFreeImageViewConstraints
{
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.msgFreeImageView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.timeLabel attribute:NSLayoutAttributeBottom multiplier:1.0 constant:0]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.msgFreeImageView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-EaseConversationCellPadding]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.msgFreeImageView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeRight multiplier:1.0 constant:-EaseConversationCellPadding]];
...
-(void) setShowMsgFreeImage:(BOOL)showMsgFreeImage
{
    if (_showMsgFreeImage != showMsgFreeImage)
    {
        _showMsgFreeImage = showMsgFreeImage;
        self.msgFreeImageView.hidden = !showMsgFreeImage;
    }
}
