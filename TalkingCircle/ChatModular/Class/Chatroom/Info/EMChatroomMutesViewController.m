...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        EMError *error = nil;
        if (buttonIndex == 0) { //移除
            weakSelf.chatroom = [[EMClient sharedClient].roomManager unmuteMembers:@[userName] fromChatroom:weakSelf.chatroom.chatroomId error:&error];
        }
...
{
    if (self.chatroom.permissionType != EMChatroomPermissionTypeAdmin && self.chatroom.permissionType != EMChatroomPermissionTypeOwner) {
        return;
    }
