...
- (void)viewDidLoad {
    ...
    //更改索引的背景颜色
    self.tableView.sectionIndexColor =DEFAULT_MAIN_COLOR;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            cell.avatarView.image = [UIImage imageNamed:@"new-friend"];
            cell.titleLabel.text = NSLocalizedString(@"title.newFriend", @"新的朋友");
        ...
        if (indexPath.row == 1) {
            cell.avatarView.image = [UIImage imageNamed:@"ground"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0) {
        if (row == 0) {
            ...
            MQNewFriendController *myFriendVC =[[MQNewFriendController alloc] init];
...
//            }
       ...
       // ((ChatViewController*)chatController).userModel = model;
#endif
