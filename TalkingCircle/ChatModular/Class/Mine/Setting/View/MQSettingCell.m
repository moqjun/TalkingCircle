...
- (void) layoutSubviews
{
    ...
    if (self.item.type == MQSettingItemTypeButton) {
    ...
    if (self.item.title) {
        ...
        if (self.item.alignment == MQSettingItemAlignmentMiddle) {
    ...
    else if (self.item.alignment == MQSettingItemAlignmentLeft) {
        float t = 105;
...
- (void) setItem:(MQSettingItem *)item
{
    _item = item;
...
// 设置数据
    ...
    if (item.type == MQSettingItemTypeSwitch) {
...
{
    NSLog(@"switchBtn.isOn:%d",switchBtn.isOn);
}
...
- (UISwitch *) cSwitch
{
    if (_cSwitch == nil) {
        ...
        //监听事件
        [_cSwitch addTarget:self action:@selector(valueChange:) forControlEvents:UIControlEventValueChanged];
    }
    return _cSwitch;
}
