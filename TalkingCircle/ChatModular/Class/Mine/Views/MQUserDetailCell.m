...
@implementation MQUserDetailCell
...
- (void) setUser:(MQUser *)user
{
    ...
    [_avatarImageView imageWithUsername:user.avatarURL placeholderImage:nil];
    ...
    if (user.userID && user.userID.length > 0) {
        [_userIDLabel setText:[NSString stringWithFormat:@"洽谈圈号：%@", user.userID]];
    }
    else {
