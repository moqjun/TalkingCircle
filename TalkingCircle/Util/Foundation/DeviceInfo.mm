...
+ (CGFloat) TotalDiskSpaceSize
{
    ...
    if(statfs("var/", &stat) >= 0){
        totalSpace = (CGFloat)(stat.f_blocks * stat.f_bsize);
    }
...
+ (CGFloat) FreeDiskSpaceSize
{
    ...
    struct statfs stat;
    if(statfs("var/", &stat) >= 0){
        freeSpace = (CGFloat)(stat.f_bsize * stat.f_bfree);
    }
...
+ (BOOL) is480hScreen
{
    if (g_hasDetectIs480hScreen == NO) {
        ...
        int height = TCScreenHeightOri(UIInterfaceOrientationPortrait);
